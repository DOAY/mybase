...
*/
		...
		$('span[data-tdtype="edit"]', $(self)).live('click', function() {
			...
			width   = $(this).width();
			var edit_acturi="";
			if($(this).attr("data-acturi")!=undefined)
			{
				edit_acturi=$(this).attr("data-acturi");
			}
			else
			{
				edit_acturi=settings.url;
			}
			$('<input type="text" class="lt_input_text" value="'+s_val+'" />').width(width).focusout(function(){
				$(this).prev('span').show().text($(this).val());
				if($(this).val() != s_val) {
					$.getJSON(edit_acturi, {id:s_id, field:s_name, val:$(this).val()}, function(result){
						if(result.status == 0) {
